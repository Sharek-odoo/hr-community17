...
class EmployeePenalty(models.Model):
        ...
        for rec in self:
            ...
            if rec.penalty_id and rec.penalty_id.penalty_type in ('deduction', 'suspend'):
                if employee_contract:
                    ...
                    hour_wage = (employee_contract.wage /30/ (employee_contract.resource_calendar_id.hours_per_day))
                    day_wage = employee_contract.wage /30

                    if rec.penalty_id.deduction_type == "percentage":
                        rec.deduction_amount = (employee_contract.wage * (rec.penalty_id.deduction_percentage / 100))
    ...
    def action_submit(self):
        ...
        if self.employee_id.work_email and self.employee_id.address_id:
            mail_details = {
                'subject': "{} Penalty".format(self.employee_id.name),
                'body': body,
                'partner_ids': [self.employee_id.address_id.id],
                'message_type': 'email',
                'email_to': self.employee_id.work_email or False
            }
            self.message_post(body=mail_details)
