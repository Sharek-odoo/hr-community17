...
class HREmployeePromotion(models.Model):
    ...
    state = fields.Selection([
        ('draft', 'Draft'),
        ('current_department_manager', 'Current Department Manager Approval'),
        ('new_department_manager', 'New Department Manager Approval'),
        ('hr', 'HR Approval'),
        ('ceo', 'CEO Approval'),
    ...
    def _check_unique_employee_effective_month(self):
        for rec in self:
            ...
            existing = self.env['hr.employee.promotion'].search(domain, limit=1)

            if existing:
                raise ValidationError(_(
                    "A promotion already exists for employee %s in the month of %s." % (
...
class HrContractPromotionLog(models.Model):
    _name = 'hr.contract.promotion.log'
    _description = 'Promotion Log for Contract'

    contract_id = fields.Many2one('hr.contract', string="Contract", ondelete='cascade')
    promotion_id = fields.Many2one('hr.employee.promotion', string="Promotion Record")
    employee_id = fields.Many2one('hr.employee', string="Employee", related='promotion_id.employee_id')
    promotion_date = fields.Date(string="Promotion Date", related='promotion_id.request_date')
...
class HrContractInherit(models.Model):
    ...
    promotion_log_ids = fields.One2many('hr.contract.promotion.log', 'contract_id', string="Promotion History")
...
class HrPayslipInherit(models.Model):
    ...
    promotion_id = fields.Many2one(
        ...
        string="Promotion",
        compute='_compute_promotion_id',
        readonly=True
    )

    @api.depends('employee_id', 'date_from', 'date_to')
    def _compute_promotion_id(self):
        for payslip in self:
            ...
            payslip.promotion_id = promotion.id if promotion else False
