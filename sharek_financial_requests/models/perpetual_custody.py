...
class PerpetualCustodyAllowance(models.Model):
    _name = 'perpetual.custody'
    _description = 'Perpetual Custody'
    ...
    custody_account_id = fields.Many2one('account.account',string="Perpetual Custody Account")
    custody_jouranl_id = fields.Many2one('account.journal',string="Perpetual Custody Journal",domain="[('type','=','general')]")
    ...
    def create_vendor_bill(self):
        for rec in self:
            ...
            # Get configuration
            account_id = self.env['ir.config_parameter'].sudo().get_param('sharek_financial_requests.perpetual_custody_account_id')

            if not account_id:
                raise ValidationError("Please configure Perpetual Custody Account in Settings.")
            ...
            lines = []
            lines.append((0, 0, {
                'name': f'Perpetual Custody for {rec.employee_id.name}',
            ...
            bill.message_post(body=f"Journal Entry created from Perpetual Custody {rec.name}")


    @api.model
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('perpetual.custody') or 'New'
    ...
    def unlink(self):
        ...
        return super(PerpetualCustodyAllowance, self).unlink()
        ...
        for rec in self.sudo():
            ...
            if direct_manager_user:
                rec.sudo()._create_activity(
                    ...
                    _("Perpetual Custody Request: Direct Manager Approval"),
    ...
    def action_direct_manager_approve(self):
        ...
        self._create_activity(users.ids,
                              _("Perpetual Custody Request: Finance Review Approval"),
    ...
    def action_finance1_approve(self):
        ...
        self._create_activity(users.ids,
                              _("Perpetual Custody Request: CEO Approval"),
                              _("Please review and approve this request."))
    ...
    def action_ceo_approve(self):
        ...
        self._create_activity(users.ids,
                              _("Perpetual Custody Request: Final Finance Approval"),
                              _("Please review and approve this request."))
    ...
    def action_finance2_approve(self):
        ...
        if self.employee_id.user_id:
            self._create_activity(self.employee_id.user_id.id,
                                  _("Perpetual Custody Request Approved"),
                                  _("Your Perpetual Custody request has been approved."))
    ...
    def action_view_vendor_bill_custody_1(self):
        self.ensure_one()
