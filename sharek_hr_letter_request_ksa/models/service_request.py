    ...
    def get_salary_intro_report_template(self):
        ...
        for rec in self:
            if rec.salary_intro_report_template and rec.id:
                fields = ['body_html']
                template_values = rec.salary_intro_report_template._generate_template([rec.id], render_fields=fields)
    ...
    def get_salary_transfer_report_template(self):
        ...
        for rec in self:
            if rec.salary_transfer_report_template and rec.id:
                fields = ['body_html']
                template_values = rec.salary_transfer_report_template._generate_template([rec.id], render_fields=fields)
    ...
    def get_letter_of_authority_report_template(self):
        ...
        for rec in self:
            if rec.letter_of_authority_report_template and rec.id:
                fields = ['body_html']
                template_values = rec.letter_of_authority_report_template._generate_template([rec.id], render_fields=fields)
    ...
    def get_experience_certificate_report_template(self):
        ...
        for rec in self:
            if rec.experience_certificate_report_template and rec.id:
                fields = ['body_html']
                template_values = rec.experience_certificate_report_template._generate_template([rec.id], render_fields=fields)
...
class ResCompany(models.Model):
    ...
    #         [('id', '=', self.env.ref('sharek_hr_letter_request_ksa.salary_intro_report_template_id').id)])
    #
    # def _get_defalut_salary_transfer_report_template(self):
    #     return self.env['mail.template'].search(
    #         [('id', '=', self.env.ref('sharek_hr_letter_request_ksa.salary_transfer_report_template_id').id)])
    #
    # def _get_defalut_letter_of_authority_report_template(self):
    #     return self.env['mail.template'].search(
    #         [('id', '=', self.env.ref('sharek_hr_letter_request_ksa.letter_of_authority_template_id').id)])
    #
    # def _get_defalut_experience_certificate_report_template(self):
    #     return self.env['mail.template'].search(
    #         [('id', '=', self.env.ref('sharek_hr_letter_request_ksa.experience_certificate_report_template_id').id)])
    ...
    salary_intro_report_template = fields.Many2one('mail.template', string='Salary Introduction Report',
                                                   domain=lambda self: [('model_id', '=', self.env.ref(
                                                       'sharek_hr_letter_request_ksa.model_letter_request').id)])
